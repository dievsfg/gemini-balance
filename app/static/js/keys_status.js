...
window.showKeyUsageDetails = async function (key) {
  ...
  const controlsHtml = `
    <div class="flex items-center gap-2 mb-3 text-xs">
      <button id="keyBtnToday" class="px-2 py-1 rounded bg-gray-200 hover:bg-gray-300 text-gray-700">今日</button>
  ...
  async function renderKeyModelUsage(key, period) {
      const container = document.getElementById('keyModelUsageTable');
      if (!container) return;
      container.innerHTML = `<div class="text-center py-4"><i class="fas fa-spinner fa-spin"></i></div>`;
      try {
          const modelUsageData = await fetchAPI(`/api/stats/key-model-usage?key=${encodeURIComponent(key)}&period=${period}`);
          const modelUsageDataArray = Object.entries(modelUsageData).map(([model_name, data]) => ({ model_name, ...data }));

          if (modelUsageDataArray.length > 0) {
              let tableRows = modelUsageDataArray.map(model => `
                  <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                      <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">${model.model_name}</td>
                      <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">${model.call_count}</td>
                      <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
          ...
          container.innerHTML = `<div class="text-center py-4 text-red-500">加载模型统计失败: ${e.message}</div>`;
      }
  }
    ...
    data.forEach((row) => {
      ...
      tableHtml += `
        <tr>
          ...
          <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
              <div title="输入 (用户到Gemini)"><i class="fas fa-arrow-down"></i> ${row.prompt_tokens === null ? 'N/A' : row.prompt_tokens}</div>
              <div title="输出 (Gemini到用户)"><i class="fas fa-arrow-up"></i> ${row.candidates_tokens === null ? 'N/A' : row.candidates_tokens}</div>
          </td>
  ...
  async function renderForPeriod(period) {
    try {
      const [details] = await Promise.all([
          fetchAPI(`/api/stats/key-details?key=${encodeURIComponent(key)}&period=${period}`),
          renderKeyModelUsage(key, period)
      ]);
  ...
  // 绑定按钮事件与默认加载
  const btnToday = document.getElementById('keyBtnToday');
  ...
  const setActive = (activeBtn) => {
    [btnToday, btn1h, btn8h, btn24h].forEach((btn) => {
      if (!btn) return;
      if (btn === activeBtn) {
        btn.classList.remove('bg-gray-200');
        btn.classList.add('bg-primary-600','text-white');
  ...
  if (btnToday) btnToday.addEventListener('click', () => { setActive(btnToday); renderForPeriod('today'); });
  ...
  if (btnToday) setActive(btnToday);
